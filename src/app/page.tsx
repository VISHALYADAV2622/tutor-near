...
export default function HomeGuestPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans">
      ...
      <nav className="bg-white border-b border-gray-100 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <Link
              href="/login"
              className="px-4 py-2 rounded-xl text-sm font-medium text-blue-600 border border-blue-300 hover:bg-blue-50 hover:border-blue-500 transition-colors"
            >
              Login
            </Link>
            <Link
              href="/register"
              className="px-4 py-2 rounded-xl text-sm font-medium bg-blue-600 text-white hover:bg-blue-700 shadow-sm transition-colors"
            >
              Register
            </Link>
          </div>
      ...
      {/* ── Hero ── */}
      <section className="bg-gradient-to-br from-blue-700 via-blue-600 to-blue-800 py-14 px-6 text-center">
        <h1 className="text-4xl font-semibold text-white leading-tight mb-3">
          Find the Best Tutors <span className="text-blue-200">Near You</span>
        </h1>
        <p className="text-base text-blue-100 mb-8">
          Search from 8,000+ verified teachers for every subject · Kanpur,
          Lucknow, Varanasi & more
        </p>
        ...
        <div className="flex items-center justify-center gap-4 flex-wrap">
          ...
          {[
            "Maths, Varanasi, Offline",
            "Physics, Online",
            "Class 10 Science",
          ].map((s) => (
            <span
              key={s}
              className="text-xs text-blue-200 flex items-center gap-1 cursor-pointer hover:text-white transition-colors"
            >
      ...
      <section className="max-w-7xl mx-auto px-6 py-10">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-3">
          {SUBJECTS.map((s) => (
            <div
              key={s.name}
              className="bg-white border border-gray-100 rounded-2xl p-3.5 cursor-pointer
      ...
      <section className="max-w-7xl mx-auto px-6 py-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {[
            {
              icon: <AiOutlineSearch className="w-6 h-6 text-blue-600" />,
              bg: "bg-blue-50",
              title: "Search nearby tutors",
              desc: "Filter by subject, location, mode, and budget to find your perfect match",
            },
            {
              icon: <AiOutlineMessage className="w-6 h-6 text-green-600" />,
              bg: "bg-green-50",
              title: "Connect & verify",
              desc: "Chat directly, check reviews, and book a free demo class before committing",
            },
            {
              icon: <AiOutlineRocket className="w-6 h-6 text-purple-600" />,
              bg: "bg-purple-50",
              title: "Start learning",
              desc: "Attend sessions online or at home, track progress, and achieve your goals",
            },
          ].map((s) => (
            <div key={s.title} className="text-center px-6">
              <div
                className={`w-14 h-14 ${s.bg} rounded-2xl flex items-center justify-center mx-auto mb-4`}
              >
                {s.icon}
              </div>
      ...
      <footer className="bg-white border-t border-gray-100 py-5 px-6">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="text-xs text-gray-400">
            © {new Date().getFullYear()} TutorNear · Privacy · Terms · Help
          </div>
        </div>
      </footer>
